    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans">
      <header className="w-full bg-[#0a1e3f] text-white h-14 flex items-center justify-between px-4 sm:px-6 shrink-0 relative z-30 border-b border-white/10">
        <div className="flex items-center gap-3">
          <button onClick={() => setIsMobileMenuOpen(true)} className="sm:hidden p-1.5 hover:bg-white/10 rounded-lg transition-colors mr-1">
            <Menu size={20} />
          </button>
        ...
        <div className="flex items-center gap-4 text-sm relative">
          <div onClick={() => setIsNotifOpen(!isNotifOpen)} className="relative flex items-center justify-center cursor-pointer p-1.5 hover:bg-white/10 rounded-full transition-colors">
            ...
            {unreadCount > 0 && <span className="absolute top-0 right-0 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white border-2 border-[#0a1e3f] animate-pulse">{unreadCount > 99 ? '99+' : unreadCount}</span>}
          </div>
          {isNotifOpen && (
            <>
              ...
              <div className="absolute top-12 right-12 w-80 bg-white rounded-xl shadow-2xl border border-slate-200 z-50 overflow-hidden flex flex-col text-slate-800">
                ...
                <div className="max-h-80 overflow-y-auto relative z-10">
                  {notifications.length === 0 ? <div className="p-6 text-center text-slate-400 text-sm">No new notifications</div> : (
                    notifications.map((notif) => (
                      <div key={notif.id} onClick={() => handleNotificationClick(notif)} className={`p-3 border-b border-slate-100 cursor-pointer hover:bg-slate-50 transition-colors ${!notif.is_read ? 'bg-blue-50/50' : 'opacity-70'}`}>
                        ...
                        <div className="flex justify-between items-center mt-2">
                          <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${notif.type === 'TICKET' || notif.type === 'MAINTENANCE' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>{notif.type}</span>
          ...
          <div className="hidden sm:block px-3 py-1.5 rounded-full text-xs font-semibold text-white border border-[#2a4d7a] bg-[#1e3a63]">Admin Portal</div>
          <button onClick={() => setIsLogoutModalOpen(true)} className="text-slate-300 hover:text-white font-medium transition-colors text-xs px-3 py-1.5 border border-transparent hover:border-slate-600 rounded-full">Log out</button>
        </div>
      </header>

      <div className="flex flex-1 overflow-hidden relative">
        {isMobileMenuOpen && <div className="fixed inset-0 bg-[#0a1e3f]/50 z-40 sm:hidden" onClick={() => setIsMobileMenuOpen(false)} />}
        <aside className={`absolute sm:static inset-y-0 left-0 z-50 w-64 bg-[#0a1e3f] text-slate-300 flex flex-col shrink-0 overflow-y-auto border-t border-white/5 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full sm:translate-x-0"}`}>
          ...
          <div className="p-4 sm:mt-2">
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="text-[10px] font-normal text-emerald-100 bg-[#359b46] px-1.5 py-0.5 rounded shadow-sm">{isLoading ? "..." : orgData?.plan || "Trial"}</span>
                <span className="text-[10px] text-slate-400 flex items-center gap-0.5">{isLoading ? "-" : orgData?.users_count || 1} managers</span>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-[#0a1e3f]/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all text-center p-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={() => setIsLogoutModalOpen(false)} className="flex-1 px-4 py-3 sm:py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition-colors border border-slate-200">Cancel</button>
              <button onClick={handleLogout} className="flex-1 bg-red-500 hover:bg-red-600 text-white px-4 py-3 sm:py-2.5 rounded-lg text-sm font-semibold transition-all shadow-sm">Log Out</button>
            </div>
          </div>
        </div>
      )}
...
function NavItem({ icon, label, isActive, onClick }: { icon: React.ReactNode, label: string, isActive: boolean, onClick: () => void }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-3 py-3 sm:py-2.5 rounded-lg text-sm font-semibold transition-all ${isActive ? "bg-[#359b46] text-white shadow-sm" : "text-slate-400 hover:bg-[#122955] hover:text-slate-200"}`}>
