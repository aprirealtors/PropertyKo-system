...
export default function ConversationTab({ orgData, adminProfile }: { orgData: any, adminProfile: any }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      // Listener para sa mga bagong send na messages (INSERT)
      ...
      // 🌟 NEW: Listener para sa status update (Kapag nag-read na yung kabila, mag-a-update to realtime!)
      ...
      if (usersData) {
        usersData.forEach(user => {
          if (user.email && user.email.trim() !== '') { 
            ...
            if (user.role === 'Property manager') { icon = Briefcase; type = 'manager'; }
            if (user.role === 'Maintenance staff') { icon = Wrench; type = 'maintenance'; }
            if (user.role === 'Tenant') { type = 'tenant'; }
            
            contactsMap.set(user.email, { 
              id: user.email, 
              name: user.name || user.email, 
              unit: user.access_level || 'No assignments',
              type: type, 
              icon: icon
            });
          }
      ...
      <div className={`w-full md:w-[360px] flex flex-col border-r border-slate-200 bg-white ${activeChat ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-0.5 custom-scrollbar">
          {isLoading ? (
            ...
            // 🌟 NEW: Pagtukoy kung anong name/label ang ilalabas sa huling chat
            const getSidebarMessagePrefix = () => {
              if (!lastMsg) return "No messages";
              if (lastMsg.sender_email === adminProfile.email) return "You: ";
              
              // Kunin ang first name ng sender (kung meron), else default to 'Them:'
              ...
              {displayedMessages.map((msg: any, idx: number) => {
                ...
                return (
                  <div key={msg.id || idx} className={`w-full flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                    ...
                    {/* 🌟 NEW: Realtime Delivered (Gray Check) & Read (Blue Check) */}
                    <div className="text-[10px] font-medium text-slate-400 mt-1 flex items-center gap-1.5">
                      ...
                      {isMe && (
                        isPending ? (
                          ...
                          <CheckCheck size={14} className="text-blue-500" /> // Read na
                        ) : (
                          <CheckCheck size={14} className="text-slate-300" /> // Delivered palang
                        )
                      )}
                    </div>
                  </div>
