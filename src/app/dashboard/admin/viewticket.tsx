  ...
  const fetchTicketsAndLiveStatuses = async () => {
    ...
    try {
      ...
      const { data: tasksData } = await supabase
        ...
        .select('id, title, location, status, assigned_to, cost, resolution_photo_url, description, priority') 
        .eq('admin_email', orgData.admin_email);
  ...
  const enrichedTickets = useMemo(() => {
    return tickets.map(ticket => {
      ...
      return {
        ...
        color,
        staffName,
        priority: liveMatch?.priority || 'Normal'
      };
    });
          ...
          <div>
            ...
            <div className="space-y-4">
              {openInProgressTasks.length === 0 ? (
                <div className="border border-dashed border-slate-300 rounded-2xl p-4 text-center text-xs text-slate-400 bg-slate-50/50">No open tasks</div>
              ) : (
                openInProgressTasks.map(t => (
                  <div 
                    ...
                    className={`bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col hover:shadow-md transition-all cursor-pointer active:scale-[0.98] ${
                      t.priority === 'Urgent' ? 'border-red-300 shadow-red-500/10' : 'border-slate-200 hover:border-[#359b46]'
                    }`}
                  >
                    {t.photo_url ? (
                      <div className="relative w-full h-32 bg-slate-100 border-b border-slate-100">
                        <img src={t.photo_url} alt="Repair issue" className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="relative w-full h-16 bg-slate-50 border-b border-slate-100 flex items-center justify-center">
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                      <div className="flex justify-between items-start mb-2 gap-2">
                        ...
                        <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border ${t.color}`}>
                          {t.label}
                        </span>
                      </div>
                      <p className="text-xs text-slate-500 mb-3 flex-1 line-clamp-2">{t.description}</p>
                      
                      <div className="flex flex-col gap-1.5 mt-auto pt-3 border-t border-slate-100 text-xs">
                        {/* ✨ FIX: Location at Priority Badge pinagtabi */}
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5 text-slate-500">
                            <MapPin size={12} className="text-slate-400" /> <span className="truncate max-w-[120px]">{t.location}</span>
                          </div>
                          {t.priority === 'Urgent' && (
                            <span className="shrink-0 px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wide bg-red-100 text-red-700 border border-red-200 animate-pulse">
                              🚨 URGENT
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-1.5 mt-1">
                          <span className="font-medium text-slate-500 border border-slate-200 bg-slate-50 px-2 py-0.5 rounded-full">
                            👤 {t.staffName}
                          </span>
                        </div>
                      </div>
              ...
              {onHoldTasks.length === 0 ? (
                ...
                onHoldTasks.map(t => (
                  <div 
                    key={t.id} 
                    onClick={() => setSelectedTicketData({ ticket: t, liveMatch: t.liveMatch, staffName: t.staffName, label: t.label, color: t.color })}
                    className={`bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col hover:shadow-md transition-all cursor-pointer opacity-90 hover:opacity-100 ${
              ...
              {resolvedTasks.length === 0 ? (
                ...
                resolvedTasks.map(t => (
                  <div 
                    key={t.id} 
                    onClick={() => setSelectedTicketData({ ticket: t, liveMatch: t.liveMatch, staffName: t.staffName, label: t.label, color: t.color })}
                    className="bg-emerald-50 rounded-2xl shadow-sm border border-emerald-100 overflow-hidden flex flex-col hover:shadow-md transition-all cursor-pointer"
                  >
                    {(t.liveMatch?.resolution_photo_url || t.photo_url) ? (
                      <div className="relative w-full h-32 bg-emerald-100/50 border-b border-emerald-100">
                        <img src={t.liveMatch?.resolution_photo_url || t.photo_url} alt="Resolved issue" className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="relative w-full h-16 bg-emerald-100/30 border-b border-emerald-100 flex items-center justify-center">
                        <span className="text-xs font-bold text-emerald-300 uppercase tracking-wider">No Photo</span>
                      </div>
                    )}
                    <div className="p-4 flex-1 flex flex-col">
                      <div className="flex justify-between items-start mb-2 gap-2">
                        <div className="flex items-start gap-2">
                          <CheckCircle2 size={16} className="text-[#359b46] shrink-0 mt-0.5" />
                      ...
                      <p className="text-xs text-slate-500 mb-3 flex-1 line-clamp-2">{t.description}</p>
                      <div className="flex flex-col gap-1.5 mt-auto pt-3 border-t border-emerald-200/50 text-xs">
                        <div className="flex items-center gap-1.5 text-slate-500">
                          <MapPin size={12} className="text-slate-400" /> <span className="truncate max-w-[120px]">{t.location}</span>
                        </div>
                        <div className="flex justify-between items-center mt-1">
                          <span className="font-medium text-slate-500 bg-white/50 px-2 py-0.5 rounded-full">
                            👤 {t.staffName}
                          </span>
                          {t.liveMatch?.cost !== undefined && t.liveMatch.cost > 0 && (
                            <span className="font-black text-[#0a1e3f]">₱{t.liveMatch.cost.toLocaleString()}</span>
                          )}
                        </div>
                      </div>
                    </div>
