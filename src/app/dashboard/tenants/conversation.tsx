...
export default function ConversationTab({
  ...
  useEffect(() => {
    // Dinagdag natin ang !isSending para mag-trigger siya ulit after mag-send
    if (activeChat && !isLoading && !isSearchActive && !isSending) {
      setTimeout(() => {
        // ✨ DITO ANG FIX: I-check muna natin kung malaki ang screen (Desktop/Tablet)
        // Kung cellphone (below 768px), hindi natin i-o-auto focus para hindi lumabas ang keyboard at hindi tumalon ang Nav Bar!
        if (window.innerWidth > 768) {
          inputRef.current?.focus();
        }
      }, 50); // Timeout para hintayin muna mag-enable ulit yung input bago i-focus
    }
    ...
    try {
      ...
      if (!error && data) {
        setMessages((prev) => {
          ...
          // Suriin kung naunang isaksak ng Realtime listener ang totoong message UUID
          const realtimeAlreadyAdded = prev.some((m) => m.id === data.id);

          if (realtimeAlreadyAdded) {
            // Kung nauna ang Realtime event, burahin na lang ang temp optimistic slot
            return prev.filter((m) => m.id !== tempId);
          }

          // Kung wala pa, ligtas na map/palitan ang tempId gamit ang totoong data mula sa database
  ...
  const isActiveRoleOnline =
    activeChat && roleEmails[activeChat]
      ? onlineUsers.includes(roleEmails[activeChat])
      : false;

  const renderRoleBadge = (roleId: string | undefined) => {
    if (roleId === "owner")
      return (
        <span className="shrink-0 text-[9px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
          Owner
        </span>
      );
    if (roleId === "manager")
      return (
        <span className="shrink-0 text-[9px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
          Manager
        </span>
      );
    if (roleId === "admin")
      return (
        <span className="shrink-0 text-[9px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
          Admin
        </span>
      );
    return null;
  ...
  return (
    <div className="absolute inset-0 flex bg-[#f8fafc] font-sans z-20 overflow-hidden pb-[80px] md:pb-0">
      {/* SIDEBAR */}
      <div
        className={`w-full md:w-[360px] flex flex-col border-r border-slate-200 bg-white ${activeChat ? "hidden md:flex" : "flex"} transition-all`}
      >
        {/* SIDEBAR HEADER */}
        <div className="shrink-0 pt-5 sm:pt-6 pb-3 sm:pb-4 px-4 sm:px-5 border-b border-slate-100 bg-white">
          <div className="flex justify-between items-center mb-3 sm:mb-4">
            <h1 className="text-xl sm:text-2xl font-black text-[#0a1e3f] tracking-tight">
              Chats
            </h1>
            <button
              onClick={() => setIsEditingNames(!isEditingNames)}
              className={`p-2 sm:p-2.5 rounded-xl transition-all border shadow-sm active:scale-95 duration-200 ${isEditingNames ? "bg-blue-50 border-blue-200 text-[#1e88e5]" : "bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100"}`}
            >
              {isEditingNames ? (
                <Check
                  size={16}
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200/80 text-[15px] md:text-sm rounded-xl sm:rounded-2xl pl-10 sm:pl-11 pr-4 py-2.5 sm:py-3 focus:outline-none focus:bg-white focus:ring-4 focus:ring-[#1e88e5]/10 focus:border-[#1e88e5] transition-all font-medium text-slate-700 placeholder:text-slate-400"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-3 space-y-1 bg-white custom-scrollbar">
          {isContactsLoading ? (
            <div className="flex flex-col items-center justify-center h-32 text-slate-400 font-bold text-[10px] sm:text-xs uppercase tracking-wider">
              <Clock className="animate-spin mb-2 text-[#1e88e5]" size={18} />{" "}
              Loading...
            </div>
                ...
                <div
                  ...
                  className={`flex items-center gap-3 sm:gap-3.5 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl cursor-pointer transition-all duration-200 relative group ${
                    isActive && !isEditingNames
                      ? "bg-blue-50/70 border border-blue-100/30 shadow-sm"
                      : "border border-transparent hover:bg-slate-50"
                  }`}
                >
                  <div className="relative shrink-0">
                    <div
                      className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-sm border transition-all duration-300 ${
                        isActive && !isEditingNames
                          ? "bg-gradient-to-br from-[#1e88e5] to-[#1565c0] text-white border-transparent shadow-blue-500/20 scale-105"
                    ...
                    {isOnline && (
                      <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-green-500 border-2 border-white rounded-full shadow-sm"></div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-[100px] sm:min-w-[120px]">
                    <div className="flex items-center justify-start flex-nowrap gap-1.5 sm:gap-2 mb-0.5 sm:mb-1 w-full whitespace-nowrap">
                      {isEditingNames ? (
                        <input
                          ...
                          className="text-[14px] sm:text-[16px] md:text-sm font-bold text-[#1e88e5] border-b-2 border-[#1e88e5] bg-transparent outline-none w-full py-0.5"
                          onClick={(e) => e.stopPropagation()}
                        />
                      ) : (
                        <>
                          <h3
                            className={`text-[13px] sm:text-[14px] font-bold tracking-tight whitespace-nowrap shrink-0 ${unreadCount > 0 ? "font-black text-[#0a1e3f]" : isActive ? "font-bold text-[#0a1e3f]" : "font-semibold text-slate-700"}`}
                          >
                    ...
                    <p
                      className={`text-[11px] sm:text-[12.5px] truncate font-medium ${unreadCount > 0 ? "font-bold text-slate-900" : "text-slate-400"}`}
                    >
                      {lastMsg ? (
                        <span>
                          <span
                  ...
                  <div className="flex flex-col items-end shrink-0 gap-1.5 sm:gap-2 min-w-[40px] sm:min-w-[45px]">
                    <span
                      className={`text-[9px] sm:text-[10px] tracking-wide font-bold ${unreadCount > 0 ? "text-[#1e88e5]" : "text-slate-400"}`}
                    >
                      {displayTime}
                    </span>
      ...
      >
        {!activeChat ? (
          ...
          <>
            ...
            <div className="shrink-0 h-[60px] sm:h-[70px] md:h-[75px] bg-white/90 backdrop-blur-md border-b border-slate-200/60 flex items-center justify-between px-3 sm:px-4 md:px-6 z-10 shadow-sm shadow-slate-100/40">
              <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
                <button
                  ...
                  className="md:hidden p-1.5 sm:p-2 text-[#1e88e5] hover:bg-slate-50 rounded-xl transition-colors active:scale-95 shrink-0"
                >
                  <ChevronLeft
                    size={20}
                    className="sm:w-[22px] sm:h-[22px]"
                ...
                <div className="relative shrink-0">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-slate-50 border border-slate-200/60 flex items-center justify-center text-slate-500 shadow-inner">
                ...
                <div className="min-w-0 flex flex-col justify-center">
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <h2 className="font-black text-[#0a1e3f] text-[14px] sm:text-[15px] md:text-[16px] truncate tracking-tight">
                      {currentChatName}
                    </h2>
              ...
              <button
                ...
                className={`p-2 sm:p-2.5 rounded-xl transition-all active:scale-95 border ${isSearchActive ? "bg-[#1e88e5] border-transparent text-white shadow-md shadow-blue-500/10" : "text-[#1e88e5] border-slate-100 hover:bg-blue-50 bg-white shadow-sm"}`}
              >
                <Search
                  size={16}
                  className="sm:w-[18px] sm:h-[18px]"
              ...
              <div className="shrink-0 bg-white border-b border-slate-200/60 p-2 sm:p-3 px-3 sm:px-5 flex items-center gap-2 sm:gap-3 z-10 shadow-sm animate-in slide-in-from-top duration-200">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl pl-8 sm:pl-10 pr-3 sm:pr-4 py-1.5 sm:py-2 text-[14px] sm:text-[16px] md:text-sm focus:outline-none focus:bg-white focus:ring-4 focus:ring-slate-500/5 transition-all text-slate-700 font-medium"
                    autoFocus
                  />
                </div>
                <button
                  ...
                  className="text-slate-400 hover:text-slate-600 text-[10px] sm:text-xs font-black uppercase tracking-wider px-2 py-1.5 sm:py-2 transition-colors"
                >
                  Cancel
                </button>
              </div>
            ...
            {/* MESSAGES SCROLL AREA (iMessage / Modern Chat UI) */}
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 bg-slate-50/50 space-y-3 sm:space-y-4 custom-scrollbar">
              {isLoading ? (
                <div className="flex justify-center items-center h-full text-slate-400 font-bold text-[10px] sm:text-xs uppercase tracking-wider gap-2">
                  <Clock
                    size={14}
                    className="animate-spin text-[#1e88e5] sm:w-4 sm:h-4"
                  />{" "}
                  Loading...
                </div>
                    ...
                    >
                      <div
                        className={`max-w-[85%] sm:max-w-[80%] md:max-w-[65%] px-3 sm:px-4 py-2 sm:py-2.5 text-[13px] sm:text-[14.5px] leading-relaxed break-words font-medium shadow-sm border ${
                          isMe
                            ? "bg-[#1e88e5] text-white border-blue-600/10 rounded-[16px] sm:rounded-[20px] rounded-br-[4px]"
            ...
            <div className="shrink-0 p-3 bg-white border-t border-slate-200 z-10">
              <form
                ...
                className="max-w-4xl mx-auto flex gap-2 sm:gap-3 items-center"
              >
                ...
                <button
                  ...
                  className={`h-[38px] w-[38px] sm:h-[42px] sm:w-[42px] rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 border transition-all active:scale-95 shadow-sm duration-200 ${
                    newMessage.trim()
                      ? "bg-[#1e88e5] text-white border-transparent shadow-blue-500/10 hover:bg-[#1565c0]"
                ...
                >
                  {isSending ? (
                    ...
                    <Send
                      size={14}
                      strokeWidth={2.5}
                      className={`sm:w-4 sm:h-4 ${newMessage.trim() ? "translate-x-0.5" : ""}`}
                    />
                  )}
                </button>
              </form>
