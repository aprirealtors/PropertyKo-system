...
const CHAT_ROLES = [
  ...
  { id: 'owner', label: 'Unit Owner', desc: 'Lease & Payment Inquiries', icon: Key },
];

export default function ConversationTab({ userData, unit }: { userData: any, unit: any }) {
    ...
    <div className="absolute inset-0 flex bg-[#f8fafc] font-sans z-20 overflow-hidden pb-[80px] md:pb-0">
      ...
      <div className={`w-full md:w-[360px] flex flex-col border-r border-slate-200 bg-white ${activeChat ? 'hidden md:flex' : 'flex'} transition-all`}>
        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-3 space-y-1 bg-white custom-scrollbar">
          {isContactsLoading ? (
            <div className="flex flex-col items-center justify-center h-32 text-slate-400 font-bold text-[10px] sm:text-xs uppercase tracking-wider"><Clock className="animate-spin mb-2 text-[#1e88e5]" size={18} /> Loading...</div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col items-end shrink-0 gap-1.5 sm:gap-2 min-w-[40px] sm:min-w-[45px]">
                    <span className={`text-[9px] sm:text-[10px] tracking-wide font-bold ${unreadCount > 0 ? 'text-[#1e88e5]' : 'text-slate-400'}`}>{displayTime}</span>
      ...
      <div className={`flex-1 flex flex-col bg-slate-50 relative ${!activeChat ? 'hidden md:flex' : 'flex'}`}>
        {!activeChat ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-slate-50">
            ...
            <p className="text-[10px] sm:text-xs text-slate-400 font-medium max-w-[200px] sm:max-w-[220px] mx-auto mt-1 leading-relaxed">Choose a caretaker or management contact from the sidebar list to view messages.</p>
          </div>
        ) : (
          <>
            {/* CHAT HEADER */}
            <div className="shrink-0 h-[60px] sm:h-[70px] md:h-[75px] bg-white/90 backdrop-blur-md border-b border-slate-200/60 flex items-center justify-between px-3 sm:px-4 md:px-6 z-10 shadow-sm shadow-slate-100/40">
              <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
                <button onClick={() => setActiveChat('')} className="md:hidden p-1.5 sm:p-2 text-[#1e88e5] hover:bg-slate-50 rounded-xl transition-colors active:scale-95 shrink-0"><ChevronLeft size={20} className="sm:w-[22px] sm:h-[22px]" strokeWidth={2.5} /></button>
                <div className="relative shrink-0">
              ...
              <button onClick={() => setIsSearchActive(!isSearchActive)} className={`p-2 sm:p-2.5 rounded-xl transition-all active:scale-95 border ${isSearchActive ? 'bg-[#1e88e5] border-transparent text-white shadow-md shadow-blue-500/10' : 'text-[#1e88e5] border-slate-100 hover:bg-blue-50 bg-white shadow-sm'}`}><Search size={16} className="sm:w-[18px] sm:h-[18px]" strokeWidth={2.5} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 bg-slate-50/50 space-y-3 sm:space-y-4 custom-scrollbar">
              {isLoading ? (
                <div className="flex justify-center items-center h-full text-slate-400 font-bold text-[10px] sm:text-xs uppercase tracking-wider gap-2"><Clock size={14} className="animate-spin text-[#1e88e5] sm:w-4 sm:h-4" /> Loading...</div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-center max-w-sm mx-auto p-4 sm:p-6">
                  ...
                  <p className="text-[10px] sm:text-xs text-slate-400 font-medium leading-relaxed mt-1">{searchQuery ? `We couldn't find "${searchQuery}" in this conversation.` : "Your messages are secure and directly sent to management."}</p>
                </div>
              ) : (
