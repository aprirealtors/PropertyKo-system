...
export default function ConversationTab({ orgData, managerProfile }: { orgData: any, managerProfile: any }) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const messagesChannelPatch = messages;
    ...
    try {
      ...
      if (!error && data) {
        setMessages(prev => {
          ...
          // Suriin kung ang realtime handler ay naunang nagpasok ng totoong data object gamit ang UUID nito
          const realtimeAlreadyAdded = prev.some(m => m.id === data.id);
          
          if (realtimeAlreadyAdded) {
            // Kung nauna ang realtime subscription, i-filter out / burahin na lang ang temp optimistic block
            return prev.filter(m => m.id !== tempId);
          }
          
          // Kung wala pa ang realtime data, ligtas na palitan ang tempId ng totoong single response data
  ...
  return (
    // ✨ FIX: Tinanggal ang pb-[80px] para sumagad ang sidebar at chat container sa pinakababa ng screen sa mobile view
      ...
      <div className={`flex-1 flex flex-col bg-slate-50 relative ${!activeChat ? 'hidden md:flex' : 'flex'}`}>
        {!activeChat ? (
          ...
          <>
            ...
            {/* INPUT AREA */}
            {/* ✨ FIX: Tinanggal ang pb-safe at pinalitan ang p-2 sm:p-3 ng p-3 para sagad at pantay ang alignment sa pinakababa ng panel */}
