...
export default function LeaseTab({ setActiveTab }: any) {
  ...
  return (
    ...
    <div className="absolute inset-0 flex flex-col bg-[var(--color-bg)] font-[family-name:var(--font-corporate)] z-20 overflow-hidden">
      ...
      <div className="shrink-0 bg-white/80 backdrop-blur-xl border-b border-[var(--color-border)] px-4 sm:px-6 py-4 sm:py-5 z-20 shadow-[var(--shadow-sm)]">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 max-w-[1600px] mx-auto w-full">
          ...
          <div className="flex justify-between items-center w-full md:w-auto">
            ...
            {/* Mobile Profile Icon */}
            <div className="md:hidden w-9 h-9 rounded-[var(--radius-sm)] bg-[var(--color-primary)]/10 text-[var(--color-primary)] flex items-center justify-center font-black text-xs shadow-inner border border-[var(--color-primary)]/30 shrink-0">
              {unit?.tenant_name 
          ...
          <div className="hidden md:flex items-center gap-3 border-l border-[var(--color-border)] pl-4 shrink-0">
            ...
            <div className="w-10 h-10 rounded-[var(--radius-md)] bg-[var(--color-primary)]/10 text-[var(--color-primary)] flex items-center justify-center font-black text-xs shadow-inner border border-[var(--color-primary)]/20 group-hover:scale-105 transition-transform duration-300">
              {unit?.tenant_name 
                ? unit.tenant_name.split(' ').map((word: string) => word.charAt(0)).join('').substring(0, 2).toUpperCase() 
                : "TE"}
            </div>
              ...
              <div className="bg-white rounded-[var(--radius-lg)] border border-[var(--color-border)] shadow-[var(--shadow-sm)] relative overflow-hidden flex flex-col lg:h-full">
                ...
                <div className="relative z-10 flex flex-col h-full overflow-y-auto custom-scrollbar p-4 sm:p-6 md:p-8">
                  <div className="flex justify-between items-center mb-5 sm:mb-6 border-b border-[var(--color-border)] pb-4 shrink-0 gap-3">
                    ...
                    <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-[var(--radius-sm)] text-[8px] sm:text-[9px] font-black uppercase tracking-widest border shadow-sm bg-[var(--color-primary)]/10 text-[var(--color-primary)] border-[var(--color-primary)]/30 shrink-0">
                      Active
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 sm:gap-4 shrink-0">
                    {/* ✨ Added Owner Display */}
                    <FormField label="Property Owner" icon={<User size={18} strokeWidth={2.5} className="w-4 h-4 sm:w-5 sm:h-5 text-[var(--color-primary)]" />} value={ownerName} />
                    <FormField label="Unit Address" icon={<Home size={18} strokeWidth={2.5} className="w-4 h-4 sm:w-5 sm:h-5 text-[var(--color-primary)]" />} value={`${propertyName} · ${unitNumber}`} />
                    ...
                    <div className="mt-3 sm:mt-4 pt-4 border-t border-[var(--color-border)]">
                      ...
                      >
                        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                          <div className={`p-2.5 sm:p-3 bg-white shadow-[var(--shadow-sm)] border rounded-[var(--radius-md)] shrink-0 ${hasDocument ? "border-[var(--color-primary)]/30 text-[var(--color-primary)]" : "border-[var(--color-border)] text-slate-400"}`}>
            ...
            <div className="w-full lg:w-[320px] xl:w-[380px] shrink-0 flex flex-col mt-4 lg:mt-0">
              <div className="bg-[var(--color-secondary)] p-5 sm:p-6 md:p-8 rounded-[var(--radius-lg)] text-white shadow-[var(--shadow-md)] relative overflow-hidden transition-all lg:h-full flex flex-col border border-[var(--color-border)]">
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  <p className="text-white/70 text-[12px] sm:text-sm font-medium leading-relaxed mb-6 sm:mb-8 shrink-0">
