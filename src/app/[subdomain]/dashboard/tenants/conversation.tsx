        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-3 space-y-1 bg-white custom-scrollbar">
          {isContactsLoading ? (
            <div className="flex flex-col items-center justify-center h-32 text-slate-400 font-bold text-[10px] sm:text-xs uppercase tracking-wider">
              <Clock className="animate-spin mb-2 text-[var(--color-primary)]" size={18} />{" "}
              Loading...
            </div>
      ...
      >
        {!activeChat ? (
          ...
          <>
              ...
              <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
                ...
                <div className="min-w-0 flex flex-col justify-center">
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <h2 className="font-black text-[var(--color-secondary)] text-[14px] sm:text-[15px] md:text-[16px] truncate tracking-tight">
                      {currentChatName}
                    </h2>
            ...
            {/* MESSAGES SCROLL AREA (Modern Chat UI) */}
                ...
                <button
                  ...
                  className={`h-[40px] w-[40px] sm:h-[48px] sm:w-[48px] rounded-[var(--radius-sm)] flex items-center justify-center shrink-0 border transition-all active:scale-95 shadow-[var(--shadow-sm)] duration-200 mb-0.5 ${
                    newMessage.trim() 
                      ...
                      : 'bg-slate-50 text-slate-300 border-[var(--color-border)] cursor-not-allowed shadow-none'
                  }`}
                >
                  {isSending ? (
