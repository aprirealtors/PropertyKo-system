...
export default function ConversationTab({
  ...
  const renderRoleBadge = (roleId: string | undefined) => {
    if (roleId === "owner")
      return (
        <span className="shrink-0 text-[9px] text-purple-700 px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
          Owner
        </span>
      );
    if (roleId === "manager")
      return (
        <span className="shrink-0 text-[9px] text-blue-700 px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
          Manager
        </span>
      );
    if (roleId === "admin")
      return (
        <span className="shrink-0 text-[9px] text-slate-600 px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
          Admin
        </span>
      );
    return null;
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    ...
                    <div className="flex justify-between items-center w-full mb-1 gap-2">
                      {/* Name & Badge Container - Ginaya ang Admin Side Format */}
                      ...
                      {/* Time - Naka shrink-0 para hindi maitulak */}
                      <span
                        className={`text-[9px] sm:text-[10px] tracking-wide shrink-0 ${
                          unreadCount > 0 ? "font-bold text-[#1e88e5]" : "font-medium text-slate-400"
                        }`}
                      >
                        {displayTime}
                      </span>
          ...
          <>
            ...
            <div className="shrink-0 p-3 sm:p-4 bg-white border-t border-slate-200/80 z-10">
              <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto flex gap-2 sm:gap-3 items-end">
                <div className="flex-1 bg-slate-50 border border-slate-200/80 rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2 sm:py-3 flex items-center min-h-[44px] sm:min-h-[48px] focus-within:bg-white focus-within:ring-4 focus-within:ring-[#1e88e5]/5 focus-within:border-slate-300 transition-all shadow-inner">
                  <textarea
                    ref={inputRef as any}
                    value={newMessage}
                    onChange={(e) => {
                ...
                <button
                  ...
                  className={`h-[40px] w-[40px] sm:h-[48px] sm:w-[48px] rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 border transition-all active:scale-95 shadow-sm duration-200 mb-0.5 ${
                    newMessage.trim() 
                      ? 'bg-[#1e88e5] text-white border-transparent shadow-blue-500/20 hover:bg-[#1565c0]' 
