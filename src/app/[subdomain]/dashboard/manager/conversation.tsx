...
export default function ConversationTab({ orgData, managerProfile }: { orgData: any, managerProfile: any }) {
  ...
  useEffect(() => {
    ...
    channel
      ...
      .on('broadcast', { event: 'typing' }, (payload) => {
        ...
        // FIX: Ensure we only show typing indicators explicitly directed to the Manager!
        if (recipient === managerProfile.email || recipient === 'manager') {
  ...
  const handleMessageChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (!isTypingRef.current && channelRef.current && activeChat) {
      ...
      channelRef.current.send({
        ...
        payload: { sender: managerProfile.email, recipient: activeChat, isTyping: true }
      });
    }
    ...
    typingTimeoutRef.current = setTimeout(() => {
      ...
      if (channelRef.current && activeChat) {
        channelRef.current.send({
          type: 'broadcast', event: 'typing',
          payload: { sender: managerProfile.email, recipient: activeChat, isTyping: false }
        });
      }
    }, 2000);
  };
  ...
  return (
    <div className="absolute inset-0 flex bg-[var(--color-bg)] font-[family-name:var(--font-corporate)] overflow-hidden pb-[80px] md:pb-0 z-20">
                  ...
                  {pinnedMessages.map((pMsg, idx) => (
                    <div key={pMsg.id} className="flex justify-between items-center gap-3">
                      <p className="text-[11px] sm:text-xs text-amber-900 font-medium truncate">
                        <span className="font-bold mr-1">{pMsg.sender_email === managerProfile.email ? 'You:' : (customNames[activeChat] || activeContactDetails?.name?.split(' ')[0] || 'User') + ':'}</span>
                        {pMsg.content}
                      </p>
                      <button 
